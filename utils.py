...
def steering_vector_AoA(theta_i, args, stream_win):
    ...
    if args.projection == "sin":
        sv = np.exp(+2j * np.pi * fc * np.sin(theta_i) * args.antenna_spacing / 3e8 * np.arange(stream_win))
...
def steering_vector_AoA_ToF(theta_i, tau_j, args, stream_win, freq_win, freq_hop=1):
        ...
        # ----------------------
        if args.projection == "sin":
            exp_phi = np.exp(+2j * np.pi * args.f_0 * np.sin(theta_i) * args.antenna_spacing / 3e8 * np.arange(stream_win))
        elif args.projection == "cos":
            exp_phi = np.exp(2j * np.pi * args.f_0 * (1 - np.cos(theta_i)) * args.antenna_spacing / 3e8 * np.arange(stream_win))
